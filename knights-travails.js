...
function buildTree(rootPosition, currentBoard) {
  let rootStr = `${rootPosition[0]}${rootPosition[1]}`;
  let possibles = possibleMoves(rootPosition, currentBoard);
  let root = new Node(rootPosition);

  for (let i = 0; i < possibles.length; i++) {
    let possibleStr = `${possibles[i][[0]]}${possibles[i][1]}`;
    root.children.push(new Node(possibles[i]));
  }
  return root;
}
function levelOrderCheck() {}

function knightMoves(startPosition, endPosition) {
  let mainBoard = new Board(startPosition);

  // buildTree example
  console.log(buildTree(startPosition, mainBoard));
